# -*- coding: utf-8 -*-
"""
Created on Wed Apr 22 21:45:10 2020
...
def f(t,y): #These are the SEIR equations. They exclude vital dynamics.
    [S,E,I,R]=y
    return [-(beta1-(L/(1+np.exp(beta2*(t-x0)))))*S*I/N, (beta1-(L/(1+np.exp(beta2*(t-x0)))))*S*I/N-sigma*E, sigma*E-gamma*I, gamma*I]
# Solving the IVP
solution = solve_ivp(f, tSolveSpace, initCond, t_eval=tEvalSpace)
tlist = solution.t
ylist = solution.y
betalist = [beta1-L/(1+np.exp(beta2*(n-x0))) for n in tlist]
...
if numOfParts>=2:
    ...
    betalist+=[beta1-L/(1+np.exp(beta2*(n-x0))) for n in range(len(solution2.t))]
...
if numOfParts>=3:
    ...
    betalist+=[beta1-L/(1+np.exp(beta2*(n-x0))) for n in range(len(solution3.t))]
