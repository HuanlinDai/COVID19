...
DataSample = DataSums.loc[StartDate:EndDate]
    ...
    # Location and COVIDTracking Data: Determines Infected and Recovered
...
# Population Data: Determines N, or population size, by location
...
initInf = ActiveInfs[0] #Initial Infected
initRec = DataSample[0] - ActiveInfs[0] #Initial Recovered
expRatio = 0.5 #Ratio of Exposed to Infected
initCond = [N-(1+expRatio)*initInf-initRec,initInf*expRatio,initInf,initRec] #Format: [S,E,I,R]
numOfParts=1
#gamma as a function of time based on the proportion of serious cases, for which the recovery time is longer
...
#Sorts a list by decreasing value
def Sort_Tuple(tup):  
    ...
    for i in range(0, lst):  
        for j in range(0, lst-i-1):  
            if (tup[j][2] > tup[j + 1][2]):  
                ...
                tup[j + 1]= temp  
    return tup
#Finds best combination of beta1 and beta by a Bayesian Optimization
